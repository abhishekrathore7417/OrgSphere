...
const STATUS_STYLE = { PRESENT:'bg-green-50 text-green-700 border-green-100', ABSENT:'bg-red-50 text-red-600 border-red-100', ON_LEAVE:'bg-amber-50 text-amber-600 border-amber-100' };
...
const TeacherAttendance = () => {
    ...
    const EMPTY = { attendanceDate: today, status: 'PRESENT', remarks: '', userId: '', checkInTime: '', checkOutTime: '' };
    ...
    const loadData = async () => {
        ...
        try {
            ...
            setTeachers(deptTeachers.map(t => ({ id: t.userId, name: t.userFullName, email: t.userEmail })));
    ...
    const openEdit = (a) => { setEditingId(a.id); setForm({ attendanceDate: a.attendanceDate||today, status: a.status, remarks: a.remarks||'', userId: a.userId||'', checkInTime: a.checkInTime ? a.checkInTime.slice(0,5) : '', checkOutTime: a.checkOutTime ? a.checkOutTime.slice(0,5) : '' }); setModal(true); };

    const handleSubmit = async (e) => {
        e.preventDefault(); setSaving(true);
        try {
            const payload = { attendanceDate: form.attendanceDate, status: form.status, remarks: form.remarks, userId: parseInt(form.userId), organizationId: parseInt(orgId), checkInTime: form.checkInTime ? `${form.checkInTime}:00` : null, checkOutTime: form.checkOutTime ? `${form.checkOutTime}:00` : null };
            if (editingId) { await companyApi.updateAttendance(editingId, payload); toast.success('Attendance updated'); }
            else { await companyApi.markAttendance(payload); toast.success('Attendance marked'); }
        ...
        <SchoolLayout>
            <div className="p-6 max-w-7xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div><h2 className="text-lg font-semibold text-gray-800">Teacher Attendance — {decoded}</h2><p className="text-sm text-gray-400 mt-0.5">Mark and track teacher daily attendance</p></div>
                ...
                : attendances.length === 0 ? <div className="bg-white rounded-xl border border-gray-200 p-16 text-center"><p className="text-sm font-medium text-gray-700">No attendance records</p></div>
                : (
                    <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
                        <table className="w-full">
                            <thead><tr className="border-b border-gray-100 bg-gray-50">{['Teacher','Date','Check In','Check Out','Status','Remarks','Action'].map(h=><th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>)}</tr></thead>
            ...
            <Modal open={modal} onClose={()=>setModal(false)} title={editingId?'Edit Attendance':'Mark Attendance'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <F label="Select Teacher *"><Select required value={form.userId} onChange={e=>setForm({...form,userId:e.target.value})}><option value="">-- Select teacher --</option>{teachers.map(t=><option key={t.id} value={t.id}>{t.fullName} ({t.email})</option>)}</Select></F>
                    ...
                    <div className="grid grid-cols-2 gap-3"><F label="Check In Time"><Input type="time" value={form.checkInTime} onChange={e=>setForm({...form,checkInTime:e.target.value})} /></F><F label="Check Out Time"><Input type="time" value={form.checkOutTime} onChange={e=>setForm({...form,checkOutTime:e.target.value})} /></F></div>
