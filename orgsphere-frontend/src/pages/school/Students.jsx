...
const EMPTY = { fullName: '', email: '', contactNumber: '', studentId: '', admissionDate: '', className: '', section: '', rollNumber: '', guardianName: '', guardianContact: '', status: 'ACTIVE', optionalFees: [] };
        ...
        try {
            if (editingId) {
                ...
                await schoolApi.createStudent({
                    ...
                    status: form.status,
                    optionalFeeTypes: form.optionalFees.join(','),
                    userId: uid, organizationId: parseInt(orgId),
                });
        ...
        <SchoolLayout>
            <div className="p-6 max-w-7xl mx-auto">
                <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                    <button onClick={() => navigate(`/school/classrooms/${classroomId}/students`)} className="hover:text-blue-600">{classroom?.classroomName || `Class-${classroomId}`}</button>
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
                    <div>
                        <h2 className="text-xl font-semibold text-gray-800">Students — {classroom?.classroomName || `Class-${classroomId}`}</h2>
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-2 lg:pb-0">
                        ...
                        <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-4 min-w-[140px] shadow-sm">
                            <div className="w-10 h-10 rounded-full bg-purple-50 text-purple-600 flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M12 14l9-5-9-5-9 5 9 5z" /><path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" /></svg>
                            </div>
                            <div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-6">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input type="text" placeholder="Search students by name, roll no, or email..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={openAdd} className="w-10 h-10 bg-blue-600 hover:bg-blue-700 text-white rounded-full flex items-center justify-center shadow-md transition-colors" title="Add Student">
                ...
                {loading ? (
                    <div className="flex justify-center h-60 items-center"><div className="w-7 h-7 border-[3px] border-blue-600 border-t-transparent rounded-full animate-spin" /></div>
                ) : students.length === 0 ? (
                    ...
                    return viewMode === 'grid' ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                            {filtered.map((s, idx) => {
                                ...
                                return (
                                    <div key={s.id} className={`bg-white rounded-2xl border ${pal.border} overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col relative group`}>
                                        <div className="p-4 flex-1">
                                            <div className="flex items-center gap-3 mb-5">
                                                ...
                                                >
                                                    <div className={`bg-white w-3.5 h-3.5 rounded-full shadow-md transform transition-transform duration-300 ${s.status === 'ACTIVE' ? 'translate-x-4' : 'translate-x-0'}`} />
                                                </button>
                                            </div>
                                            ...
                                            <div className="space-y-2 mb-2 px-1">
                                                ...
                                                <div className="flex items-center text-[13px] text-gray-700">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 mr-2 shrink-0 ${pal.icon}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                                    <span className="w-24 text-gray-600 font-medium">Session:</span>
                                                    <span className="">{classroom?.session || '—'}</span>
                                                </div>
                                                <div className="flex items-center text-[13px] text-gray-700">
                                                    <span className={`h-4 w-4 mr-2 shrink-0 font-medium flex items-center justify-center ${pal.icon}`}>#</span>
                                                    <span className="w-24 text-gray-600 font-medium">Roll No.:</span>
                                                    <span className="">{s.rollNumber || '—'}</span>
                                                </div>
                                                <div className="flex items-center text-[13px] text-gray-700">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 mr-2 shrink-0 ${pal.icon}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                                        ...
                                        <div className="p-3 pt-0 mt-2">
                                            <button onClick={(e) => { e.stopPropagation(); openEdit(s); }} className={`w-full flex items-center justify-center gap-1.5 py-2 bg-white ${pal.text} ${pal.hoverBg} border ${pal.border} text-xs font-medium rounded-xl transition-colors`}>
                                    ...
                                    {filtered.map(s => (
                                        <tr key={s.id} className="hover:bg-blue-50/30 transition-colors">
                                            ...
                                            <td className="px-4 py-3.5 text-sm">
                                                <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${s.status === 'ACTIVE' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-gray-50 text-gray-500 border-gray-200'}`}>
                                                    {s.status}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3.5 text-sm text-gray-500">{s.guardianName || '—'}</td>
                                            <td className="px-4 py-3.5">
                                                <button onClick={() => openEdit(s)} title="Edit" className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 hover:text-blue-600 transition-colors">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                                </button>
                                            </td>
                                        </tr>
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    ...
                    <div className="pt-2 border-t border-gray-100">
                        <div className="mb-2">
                            ...
                            <p className="text-[10px] text-gray-400">School & Exam fees are compulsory. Select other fees applicable to this student.</p>
                        </div>
                        <div className="grid grid-cols-3 gap-2">
                            {['TRANSPORT', 'LIBRARY', 'SPORTS', 'HOSTEL', 'OTHER'].map(f => (
                                <label key={f} className="flex items-center gap-2 text-xs font-medium text-gray-600 bg-gray-50 px-3 py-2 rounded-lg cursor-pointer hover:bg-gray-100">
                                    <input 
                                        type="checkbox" 
