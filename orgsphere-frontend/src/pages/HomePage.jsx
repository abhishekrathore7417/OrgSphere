...
const WHY_CARDS = [
  { icon: ShieldIcon, title: 'Secure & Reliable', desc: 'Enterprise-grade security with data encryption and role-based access' },
  { icon: TrendingUpIcon, title: 'Real-time Analytics', desc: 'Track performance with live dashboards and custom reports' },
  { icon: UsersIcon, title: '24/7 Support', desc: 'Dedicated support team for your organization, always available' },
  { icon: SparklesIcon, title: 'Scalable Solution', desc: 'Grow your business without limits, from startups to enterprises' },
];

const TESTIMONIALS = [
  { text: 'OrgSphere transformed our school management. Fee collection and attendance tracking are now a breeze!', name: 'Rajesh Kumar', role: 'Principal, ABC School', initials: 'RK' },
  { text: 'We manage 200+ employees across 6 departments with ease. The payroll feature is a lifesaver.', name: 'Priya Sharma', role: 'HR Manager, XYZ Company', initials: 'PS' },
  { text: 'The user-friendly interface and 24/7 support make OrgSphere the best choice for our organization.', name: 'Amit Verma', role: 'CTO, TechStart Inc.', initials: 'AV' },
];
...
const FAQS = [
  { q: 'What is OrgSphere?', a: 'OrgSphere is a complete management solution for companies and schools, offering features like employee/student management, attendance, fees, leaves, and payroll.' },
  { q: 'Is it free?', a: 'Yes, we offer a 7-day free trial. After that, choose a plan that fits your organization\'s needs.' },
  { q: 'Can I switch plans?', a: 'Yes, you can upgrade or downgrade your plan anytime from your account settings.' },
  { q: 'Is my data secure?', a: 'Absolutely. We use enterprise-grade encryption, regular backups, and role-based access to keep your data safe.' },
      ...
      `}</style>
        ...
        >
          ...
          >
            ...
            <span className="flex h-8 w-8 items-center justify-center rounded-lg" style={{ background: 'linear-gradient(135deg, #7c3aed, #4f46e5)' }}>
            ...
            <div className="hidden md:flex items-center gap-2">
              <Link
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(0,0,0,0.05)'; e.currentTarget.style.color = '#1f2937' }}
          ...
          {/* mobile menu */}
          {menuOpen && (
              <div className="absolute top-20 left-4 right-4 p-5 flex flex-col gap-3 rounded-2xl" style={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(0,0,0,0.08)', backdropFilter: 'blur(12px)' }}>
                {NAV_LINKS.map(l => <a key={l.href} href={l.href} className="text-sm hover:text-violet-600" style={{ color: '#4b5563' }} onClick={() => setMenuOpen(false)}>{l.label}</a>)}
                <Link to="/login" className="text-sm hover:text-violet-600" style={{ color: '#4b5563' }} onClick={() => setMenuOpen(false)}>Sign in</Link>
                <button onClick={() => setRegisterOpen(true)} className="text-sm font-semibold rounded-full py-2 text-center transition-all hover:scale-[1.02]" style={{ background: 'linear-gradient(135deg, #7c3aed, #4f46e5)', color: '#fff' }} onClick={() => setMenuOpen(false)}>
                  Get Started
                </button>
              </div>
          )}
        ...
        {/* ============================= 2. HERO ============================= */}
        <header className="relative overflow-hidden bg-gradient-to-b from-violet-50 via-white to-white pt-32 pb-20 sm:pt-40">
          {/* decorative blobs */}
          <div className="pointer-events-none absolute -top-20 -left-24 h-72 w-72 rounded-full bg-violet-300/40 blur-3xl animate-blob" />
          <div className="pointer-events-none absolute top-10 right-0 h-72 w-72 rounded-full bg-indigo-300/40 blur-3xl animate-blob" style={{ animationDelay: '3s' }} />

          <div className="relative mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
          <span className="inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white/70 px-4 py-1.5 text-sm font-medium text-violet-700 shadow-sm backdrop-blur">
            ...
            <p className="mx-auto mt-6 max-w-2xl text-pretty text-lg leading-relaxed text-slate-600">
              OrgSphere is the complete management solution for companies and schools. Streamline your operations with our powerful SaaS platform.
            </p>
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
              ...
              <button className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-8 py-3.5 text-base font-semibold text-slate-700 shadow-sm transition-colors hover:bg-slate-50">
            ...
            <div className="mx-auto mt-14 max-w-4xl animate-fade-up">
              <div className="rounded-3xl border border-slate-200 bg-white p-3 shadow-2xl shadow-violet-500/10">
        ...
        <section className="relative z-10 -mt-10 px-4 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-6xl grid-cols-2 gap-4 rounded-3xl border border-slate-100 bg-white p-6 shadow-xl sm:gap-6 lg:grid-cols-4">
            {STATS.map((stat) => (
                <div key={stat.label} className="flex flex-col items-center gap-2 rounded-2xl p-4 text-center transition-colors hover:bg-violet-50">
          ...
          <div key={activeTab} className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {currentTab.features.map((f, i) => {
              ...
              return (
                  ...
                  >
                <span className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-violet-100 text-violet-600 transition-colors group-hover:bg-violet-600 group-hover:text-white">
        ...
        <section className="bg-slate-50 py-24">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-2xl text-center">
              <h2 className="text-balance text-3xl font-extrabold sm:text-4xl">Get Started in 3 Simple Steps</h2>
            </div>

            <div className="mt-16 grid grid-cols-1 gap-10 md:grid-cols-3">
              {STEPS.map((step, i) => (
                  <div key={step.num} className="relative flex flex-col items-center text-center">
                <span className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-indigo-600 text-2xl font-extrabold text-white shadow-lg shadow-violet-500/30">
                  {step.num}
                </span>
                    {i < STEPS.length - 1 && (
                        <ArrowRightIcon className="absolute top-6 left-[calc(50%+3.5rem)] hidden h-8 w-8 text-violet-300 md:block" />
                    )}
            ...
            {WHY_CARDS.map((card, i) => {
              ...
              return (
                  <div
                      ...
                      style={{ animationDelay: `${i * 0.08}s` }}
                  >
                <span className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600 text-white shadow-lg shadow-violet-500/30">
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-3">
              {TESTIMONIALS.map((t, i) => (
                  ...
                  <span className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-indigo-600 text-sm font-bold text-white">
                    {t.initials}
                  </span>
                      <div>
        ...
        <section id="pricing" className="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-balance text-3xl font-extrabold sm:text-4xl">Start Your 7-Day Free Trial Today!</h2>
            <p className="mt-4 text-lg text-slate-600">
          ...
          <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-3">
            {PLANS.map((plan) => (
                ...
                >
                  {plan.highlighted && (
                      <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-amber-400 px-4 py-1 text-xs font-bold text-amber-950 shadow">
                  MOST POPULAR
                </span>
                  )}
                  {plan.badge && !plan.highlighted && (
                      <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-violet-100 px-4 py-1 text-xs font-bold text-violet-700">
                  {plan.badge}
                </span>
                  )}
                  ...
                  <div className="mt-4 flex items-end gap-1">
                    ...
                    <span className={`mb-1 text-sm ${plan.highlighted ? 'text-violet-100' : 'text-slate-500'}`}>{plan.period}</span>
                  </div>

                  <ul className="mt-6 flex-1 space-y-3">
                    {plan.features.map((feat) => (
                        <li key={feat} className="flex items-center gap-2 text-sm">
                          <CheckIcon className={`h-5 w-5 shrink-0 ${plan.highlighted ? 'text-amber-300' : 'text-violet-600'}`} />
                          <span className={plan.highlighted ? 'text-violet-50' : 'text-slate-600'}>{feat}</span>
                        </li>
                    ))}
                  </ul>
                ...
                return (
                    <div key={faq.q} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                      ...
                      >
                        ...
                        <ChevronDownIcon className={`h-5 w-5 shrink-0 text-violet-600 transition-transform duration-300 ${open ? 'rotate-180' : ''}`} />
                      </button>
            ...
            <div className="grid grid-cols-2 gap-8 sm:grid-cols-3 lg:grid-cols-5">
              ...
              <div className="col-span-2 sm:col-span-3 lg:col-span-1">
                ...
                <div className="mt-5 flex gap-3">
                  <a href="#contact" className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-800 text-slate-300 transition-colors hover:bg-violet-600 hover:text-white"><LinkedinIcon className="h-4 w-4" /></a>
                  <a href="#contact" className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-800 text-slate-300 transition-colors hover:bg-violet-600 hover:text-white"><TwitterIcon className="h-4 w-4" /></a>
                  <a href="#contact" className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-800 text-slate-300 transition-colors hover:bg-violet-600 hover:text-white"><YoutubeIcon className="h-4 w-4" /></a>
                  <a href="#contact" className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-800 text-slate-300 transition-colors hover:bg-violet-600 hover:text-white"><GithubIcon className="h-4 w-4" /></a>
                </div>
              </div>
              ...
              {FOOTER_COLS.map((col) => (
                  <div key={col.title}>
                    ...
                    <ul className="mt-4 space-y-3">
                      {col.links.map((link) => (
                          <li key={link}>
                            <a href="#contact" className="text-sm text-slate-400 transition-colors hover:text-violet-400">
                              {link}
                            </a>
                          </li>
                      ))}
              ...
              <div>
                ...
                <ul className="mt-4 space-y-3 text-sm text-slate-400">
                  <li className="flex items-center gap-2"><MailIcon className="h-4 w-4" /> hello@orgsphere.com</li>
                  <li className="flex items-center gap-2"><AwardIcon className="h-4 w-4" /> ISO 27001 Certified</li>
                  <li className="flex items-center gap-2"><ClipboardListIcon className="h-4 w-4" /> GDPR Compliant</li>
                </ul>
              </div>
            </div>
        ...
        {registerOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setRegisterOpen(false)} />
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-7">
              ...
              <button
                onClick={() => { navigate('/register/company'); setRegisterOpen(false); }}
              ...
              >
                <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-violet-100 text-violet-600 group-hover:bg-violet-600 group-hover:text-white transition-all shrink-0">
              ...
              <button
                onClick={() => { navigate('/register/school'); setRegisterOpen(false); }}
              ...
              <button onClick={() => setRegisterOpen(false)} className="mt-4 w-full text-sm text-slate-400 hover:text-slate-600">
                Cancel
              </button>
            </div>
          </div>
